...
for i = 1:size(Occupied,2) %1 to number of occupied boxes
    Q_row_next_state(Occupied(i))=0; %The zeros correspond to invalid actions and they must have value zero
end
%state1
%Q_row_next_state
for i = 1:9
...
%Q_Table(state1,:) = Q_row_next_state;
% columns = [1 2 3 4 5 6 7 8 9];
% index = sub2ind(size(Q_Table),state1,columns);
% Q_Table(index) = Q_row_next_state(columns);
end
